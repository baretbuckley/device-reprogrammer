...
pub const Setup = struct {
    ...
    // This function creates the virtual device with the given name and the keys that have been
    // set with setKeys. It returns a VirtualDevice that can be used to emit events to the device.
    // The Setup struct can not be further used after constructing a virtual device.
    pub fn createDevice(self: *const Setup, name: []const u8) !VirtualDevice {
        const fd = self.file_handle.handle;
        var setup = input.uinputSetup{
            .id = .{
        ...
        @memmove(setup.name[0..name.len], name);
        input.ui_dev_setup(fd, &setup) catch {
            return error.DevSetupFailed;
        };
        input.ui_dev_create(fd) catch {
            return error.DevCreateFailed;
        };
        return VirtualDevice{ .file_handle = self.file_handle };
    }
};

pub fn init(io: std.Io, name: []const u8) !VirtualDevice {
    _ = name;
    const file_handle = try std.Io.Dir.cwd().openFile(io, "/dev/uinput", .{ .mode = .write_only, .lock_nonblocking = true });
    const fd = file_handle.handle;
    input.ui_set_evbit(fd, codes.Type.Key) catch {
    ...
    std.mem.copyForwards(u8, setup.name[0..], "zig-virtual-device");
    ...
    const device = VirtualDevice{ .file_handle = file_handle };
    return device;
}

pub fn destroy(self: *VirtualDevice, io: std.Io) void {
    self.file_handle.close(io);
}

pub fn emitEvent(self: *const VirtualDevice, event: input.InputEvent) !void {
    const fd = self.file_handle.handle;
    const bytes_written = linux.write(fd, std.mem.asBytes(&event), @sizeOf(input.InputEvent));

    if (linux.errno(bytes_written) != .SUCCESS or bytes_written != @sizeOf(input.InputEvent)) {
        std.debug.print("Error emiting event: {}\n", .{@intFromEnum(linux.errno(bytes_written))});
        return error.WriteFailed;
    }
    std.debug.print("Emitted event: type={d}, code={d}, value={d}\n", .{ event.event_type, event.code, event.value });
}

pub fn emitEvents(self: *const VirtualDevice, events: []const input.InputEvent) !void {
    const fd = self.file_handle.handle;
    const bytes_written = linux.write(fd, std.mem.sliceAsBytes(events), @sizeOf(input.InputEvent) * events.len);

    if (linux.errno(bytes_written) != .SUCCESS or bytes_written != @sizeOf(input.InputEvent) * events.len) {
        std.debug.print("Error emiting events: {}\n", .{@intFromEnum(linux.errno(bytes_written))});
        return error.WriteFailed;
    }
...
pub fn emitKeyEvent(self: *const VirtualDevice, key: codes.KeyEvent, value: i32) !void {
    const event = input.InputEvent{
        .time = linux.timeval{ .sec = 0, .usec = 0 },
        .event_type = codes.Type.Key,
        .code = key.code,
        .value = value,
    };
...
pub fn emitSyncEvent(self: *const VirtualDevice) !void {
    const event = input.InputEvent{
        .time = linux.timeval{ .sec = 0, .usec = 0 },
        .event_type = codes.Type.Sync,
        .code = 0,
        .value = 0,
    };
    try self.emitEvent(event);
